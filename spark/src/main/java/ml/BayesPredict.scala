...
object BayesPredict {
    ...
    def train() = {
        ...
        val native = new NaiveBayes().setFeaturesCol("features").setLabelCol("label").setModelType("multinomial")
        val pipeLine = new Pipeline().setStages(Array(fenci, hashingTf, idf, native))

        val paramGrid = new ParamGridBuilder().addGrid(hashingTf.numFeatures, Array(2000, 4000, 8000, 12000, 16000, 20000, 24000, 29000))
            .build()
