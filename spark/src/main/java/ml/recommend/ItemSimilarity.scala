...
object ItemSimilarity {
  ...
  def CooccurrenceSimilarity(user_rdd: RDD[ItemPref]): (RDD[ItemSimi]) = {
    // 0 数据做准备
    val user_rdd1 = user_rdd.map(f => (f.userid, f.itemid, f.pref))  // (用户，物品，评分)
    ...
    // 3 对角矩阵 
    val user_rdd6 = user_rdd5.filter(f => f._1._1 == f._1._2) //  相同物品的同现记录   一倍的同现次数
    // 4 非对角矩阵 
    val user_rdd7 = user_rdd5.filter(f => f._1._1 != f._1._2) // 不同物品的同现记录  单倍同现次数，正反二倍的同现次数
  ...
  def EuclideanDistanceSimilarity(user_rdd: RDD[ItemPref]): (RDD[ItemSimi]) = {
    ...
    def logLikelihoodSimilar(user_rdd: RDD[ItemPref]): (RDD[ItemSimi]) ={
        println("log比相似度")
        val data1 = user_rdd.map(x => (x.userid,(x.itemid,1))) // (userid,(itemid,1))
        val userCounts = data1.reduceByKey((x,y) => x).count()  // numUsers
        val itemCounts = user_rdd.map(x => (x.itemid,1)).reduceByKey(_+_) // itemId,userCount
