...
class RecommendedItem {
  ...
  def Recommend(items_similar: RDD[ItemSimi],
    ...
    //   1 矩阵计算——i行与j列join
    val rdd_app1_R2 = rdd_app1_R1.map(f => (f._1, (f._2, f._3))) //( itemid1 , (itemid2 , similar))
                                  .join(user_prefer1.map(f => (f._2, (f._1, f._3)))) //(物品,(用户,评分))
    //   2 矩阵计算——i行与j列元素相乘
    val rdd_app1_R3 = rdd_app1_R2.map(f => ((f._2._2._1, f._2._1._1), f._2._2._2 * f._2._1._2)) // ((用户,物品),评分 * 相似度)
    //   3 矩阵计算——用户：元素累加求和
      val rdd_sum = rdd_app1_R2.map(f => ((f._2._2._1, f._2._1._1),f._2._1._2)).reduceByKey((x,y)=>x+y) // ((用户,物品), 相似度和)
    var rdd_app1_R4 = rdd_app1_R3.reduceByKey((x, y) => x + y) // ((用户,物品),∑评分 * 相似度)
      rdd_app1_R4 = rdd_app1_R4.join(rdd_sum).map(x=>(x._1,x._2._1/x._2._2))
    //   4 矩阵计算——用户：对结果过滤已有I2
    ...
    val rdd_app1_R6 = rdd_app1_R5.groupByKey() // (用户,((物品,评分),(物品,评分)、、、))
