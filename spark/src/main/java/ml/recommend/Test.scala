...
object ItemCF {
  def main(args: Array[String]) {
    ...
    //1 读取样本数据
//    val data_path = "spark/data/sample_itemcf2.txt"
    val data_path = "spark/data/move.csv"
    val data = sc.textFile(data_path)
    val userdata = data.map(_.split(",")).map(f => (ItemPref(f(0), f(1), f(2).toDouble))).cache()

    //2 建立模型
    val mysimil = new ItemSimilarity()
    val simil_rdd1 = mysimil.Similarity(userdata, "logLikelihoodSimilar") //logLikelihoodSimilar   cooccurrence
      simil_rdd1.foreach(println(_))
