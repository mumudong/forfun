...
public class C3POUtil {
    ...
    static{
        try {
            ...
            ds.setMaxPoolSize(2);//设置连接池的最大连接数
            ds.setMinPoolSize(1);//设置连接池的最小连接数
            ds.setInitialPoolSize(1);//设置连接池的初始连接数
            ds.setMaxStatements(0);//设置连接池的缓存Statement的最大数
...
//        }
    ...
    public static void main(String[] args) throws Exception{
        ...
        try{
//            pst = conn.prepareStatement("select count(id) from creditrating");
//            rsd  = pst.execute();
//            if(rsd.next())
//                System.out.println("数据条数----》" +rsd.getInt(1));
