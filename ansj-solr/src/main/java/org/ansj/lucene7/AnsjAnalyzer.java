...
public class AnsjAnalyzer extends Analyzer {
    ...
    public static enum TYPE {
        base_ansj, index, query, dic_ansj, nlp_ansj
    }
    ...
    public static Tokenizer getTokenizer(Reader reader, Map<String, String> args) {
        ...
        DicLibrary.put("dic","/opt/mu_test/apache-tomcat-9.0.10/webapps/solr/WEB-INF/classes/ext.dic");

        if (type == null) {
            type = AnsjAnalyzer.TYPE.base_ansj.name();
        }
        LOG.error("呼叫mumudong args --> " + args);
        switch (AnsjAnalyzer.TYPE.valueOf(type)) {
            case base_ansj:
                LOG.error("呼叫mumudong base_ansj init...");
                analysis = new BaseAnalysis();
                break;
            case index:
                LOG.error("呼叫mumudong index init...");
                analysis = new IndexAnalysis();
                break;
            case dic_ansj:
                LOG.error("呼叫mumudong dic_ansj init...");
                analysis = new DicAnalysis();
                break;
            case query:
                LOG.error("呼叫mumudong query init...");
                analysis = new ToAnalysis();
                break;
            case nlp_ansj:
                LOG.error("呼叫mumudong nlp_ansj init...");
                ...
                break;
            default:
                LOG.error("呼叫mumudong default init...");
                analysis = new BaseAnalysis();
        }
