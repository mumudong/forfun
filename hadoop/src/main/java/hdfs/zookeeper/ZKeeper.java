...
public class ZKeeper implements Watcher{
    ...
    public void process(WatchedEvent event) {
        ...
        System.out.println((event.getType())+"-------------process（）中打印状态");  // 打印状态
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.println("是否存在路径:" + test.isexist(path));
        test.create(path);

        System.out.println("路径是否存在->" + test.isexist(path));
        test.write(path, "test");
        System.out.println("路径是否存在-->" + test.isexist(path));
        String result = test.read(path, test.wh);
        System.out.println("读取数据-->" + path + " value = " + result);
